...
def create_row(table, curs, con):
    ...
    link_id = input("Enter the advertisement ID: ")
    link = f"https://krisha.kz/a/show/{link_id}"

    try:
        curs.execute(f"INSERT INTO offers (title, price, address, link_id, link) VALUES ('{title}', '{price}', '{address}', '{link_id}', '{link}');")
        con.commit()
...
def change_row(table, curs, con):
    ...
    try:
        curs.execute(f"UPDATE offers SET title = '{title}', price = '{price}', address = '{address}', link_id = '{link_id}', link = '{link}' WHERE id = {id};")
        con.commit()
...
def load_json(table, json_file, curs, con):
    with open(json_file, 'r') as data:
        ...
        for item in extracted_data:
            try:
                curs.execute(f"""
                INSERT INTO offers (title, price, address, link_id, link)
